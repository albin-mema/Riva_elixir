defmodule RivaAsh.Resources.RecurringReservation do
  ...
  actions do
    ...
    create :create do
      accept [
        :client_id, :item_id, :employee_id, :start_date, :start_time, :end_time,
        :consecutive_days, :pattern_type, :notes, :title
      ]
      primary? true
    end
    ...
    # TODO: Add action to generate individual reservation instances
    # action :generate_instances, :struct do
    #   constraints instance_of: RivaAsh.Resources.RecurringReservation
    #   argument :id, :uuid, allow_nil?: false
    #   run RivaAsh.Reactors.RecurringReservationGenerator
    # end
  end
  ...
  calculations do
    ...
    calculate :confirmed_instances, :integer,
      expr(count(instances, query: [filter: expr(status == "confirmed")])) do
      public? true
      description "Number of confirmed instances"
    end

    calculate :end_date, :date,
      expr(date_add(start_date, consecutive_days - 1, "day")) do
      public? true
