...
module MuseumPropertyTests =

    [<Property(MaxTest = 50)>]
    let ``Capacity monotonicity (timed entry)`` (cap: NonNegativeInt, d: NonNegativeInt) =
        let cap' = max 1 cap.Get
        let delta = d.Get % 20
        let rules = [| Dsl.capacityRuleDsl "MUS-CAP-PROP" "participants" "resourceAvailability.entry-10am.capacity" |]
        let mk p =
            let r = JsonDocument.Parse($"{{ \"participants\": {p} }}").RootElement.GetRawText()
            let res = Some ($"{{ \"resourceAvailability\": {{ \"entry-10am\": {{ \"capacity\": {cap'} }} }} }}")
    ...
    [<Property(MaxTest = 50)>]
    let ``Lead time: 24h equality passes and monotonic failset`` (lead: NonNegativeInt) =
        let leadVal = lead.Get % (72*60)
        let run (min:int) (leadValue:int) =
            let rules = [| Dsl.leadTimeRuleDsl "MUS-LEAD-PROP" "leadMinutes" min |]
            let r = JsonDocument.Parse($"{{ \"leadMinutes\": {leadValue} }}").RootElement.GetRawText()
            Dsl.executeRulesWithData rules r None
        let eq = run (24*60) (24*60)
        let low = run (12*60) leadVal
        let high = run (48*60) leadVal
