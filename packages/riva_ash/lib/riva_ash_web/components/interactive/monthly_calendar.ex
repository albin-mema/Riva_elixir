defmodule RivaAshWeb.Components.Interactive.MonthlyCalendar do
  ...
  """
  @spec validate_date_lists(list(String.t()), list(String.t())) :: {:ok, {list(String.t()), list(String.t())}} | {:error, String.t()}
  defp validate_date_lists(selectable_dates, disabled_dates) when is_list(selectable_dates) and is_list(disabled_dates) do
  ...
  defp validate_date_list(dates, list_name) when is_list(dates) do
    case Enum.find(dates, fn date -> not is_valid_date(date) end) do
      nil -> {:ok, dates}
  ...
  ## Examples
      ...
      iex> is_valid_date("2024-01-15")
      true
      
      iex> is_valid_date("invalid-date")
      false
  """
  @spec is_valid_date(String.t()) :: boolean()
  defp is_valid_date(date) when is_binary(date) do
    # Simple date validation - in production, use a proper date library
    String.match?(date, ~r/^\d{4}-\d{2}-\d{2}$/)
  end

  defp is_valid_date(_date) do
    false
  end
    ...
    <div class={["monthly-calendar", @class]} {@rest}>
      ...
      <div class="calendar-grid">
        ...
        <div :for={day <- generate_calendar_days(@current_date)} class={[
          "calendar-day",
          if(is_today(day, @current_date), do: "today", else: ""),
          if(is_selected_date(day, @selectable_dates), do: "selectable", else: ""),
          if(is_disabled_date(day, @disabled_dates), do: "disabled", else: ""),
          if(has_events(day, @events), do: "has-events", else: "")
        ]}>
          <button
            :if={not is_disabled_date(day, @disabled_dates)}
            phx-click={@on_date_click}
            phx-value-date={day}
            class="day-button"
            disabled={is_disabled_date(day, @disabled_dates)}
          >
  ...
  defp get_days_in_month(year, month) when is_integer(year) and is_integer(month) and month >= 1 and month <= 12 do
    ...
    case month do
      2 -> if is_leap_year(year), do: 29, else: 28
      4 -> 30
      6 -> 30
      9 -> 30
      11 -> 30
  ...
  ## Examples
      ...
      iex> is_leap_year(2024)
      true
      
      iex> is_leap_year(2023)
      false
  """
  @spec is_leap_year(integer()) :: boolean()
  defp is_leap_year(year) when is_integer(year) do
    (rem(year, 400) == 0) or (rem(year, 100) != 0 and rem(year, 4) == 0)
  end

  defp is_leap_year(_year) do
    false
  end
  ...
  ## Examples
      ...
      false
  """
  @spec is_today(String.t(), String.t()) :: boolean()
  defp is_today(day, current_month) when is_binary(day) and is_binary(current_month) do
    # Simple implementation - in production, use a proper date library
    String.starts_with?(day, current_month)
  end

  defp is_today(_day, _current_month) do
    false
  end
  ...
  ## Examples
      ...
      false
  """
  @spec is_selected_date(String.t(), list(String.t())) :: boolean()
  defp is_selected_date(day, selectable_dates) when is_binary(day) and is_list(selectable_dates) do
    day in selectable_dates
  end

  defp is_selected_date(_day, _selectable_dates) do
    false
  end
  ...
  ## Examples
      ...
      false
  """
  @spec is_disabled_date(String.t(), list(String.t())) :: boolean()
  defp is_disabled_date(day, disabled_dates) when is_binary(day) and is_list(disabled_dates) do
    day in disabled_dates
  end

  defp is_disabled_date(_day, _disabled_dates) do
    false
  end
