defmodule RivaAsh.Payment.PaymentService do
  ...
  def validate_payment_attrs(attrs) do
    ...
    client_id_errors = 
      case attrs do
        %{client_id: client_id} when is_binary(client_id) and byte_size(client_id) > 0 ->
          []
        _ ->
    ...
    currency_errors = 
      case attrs do
        %{currency: currency} when is_binary(currency) and byte_size(currency) > 0 ->
          []
        _ ->
    ...
    status_errors = 
      case attrs do
        %{status: status} when status in [:pending, :completed, :failed, :refunded, :cancelled] ->
          []
        %{status: _status} ->
          ["Invalid status"]
        _ ->
    ...
    all_errors = client_id_errors ++ amount_errors ++ currency_errors ++ status_errors
  ...
  def within_amount_limits?(amount) do
    ...
    max_amount = Application.get_env(:riva_ash, :max_payment_amount, 10000.00)
