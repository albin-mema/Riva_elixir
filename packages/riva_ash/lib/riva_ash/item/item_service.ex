defmodule RivaAsh.Item.ItemService do
  ...
  def create_item(item_params, user) do
    with :ok <- validate_item_params(item_params),
         {:ok, item} <- Item.create!(item_params, actor: user) do
      {:ok, item}
    end
  end
  ...
  def update_item(item_id, update_params, user) do
    with {:ok, item} <- get_item(item_id, user),
         ...
         {:ok, updated_item} <- Item.update!(item, update_params, actor: user) do
      {:ok, updated_item}
    end
  end
  ...
  def delete_item(item_id, user) do
    with {:ok, item} <- get_item(item_id, user),
         ...
         {:ok, deleted_item} <- Item.destroy!(item, actor: user) do
      {:ok, deleted_item}
    end
  end
  ...
  defp validate_update_params(item, update_params) do
  ...
  defp has_active_reservations_or_holds(item) do
