defmodule RivaAsh.Layout.LayoutService do
  ...
  def validate_layout_attrs(attrs) do
    errors =
      ...
      |> validate_width(attrs)
      |> validate_height(attrs)
  ...
  defp validate_width(errors, %{width: width}) when is_integer(width) and width > 0, do: errors
  defp validate_width(errors, %{width: width}) when is_integer(width), do: ["Width must be greater than 0" | errors]
  defp validate_width(errors, _), do: ["Width is required and must be a positive integer" | errors]

  defp validate_height(errors, %{height: height}) when is_integer(height) and height > 0, do: errors
  defp validate_height(errors, %{height: height}) when is_integer(height), do: ["Height must be greater than 0" | errors]
  defp validate_height(errors, _), do: ["Height is required and must be a positive integer" | errors]
  ...
  def get_layout_stats(business_id) do
    ...
    with {:ok, [%{count: total_count}]} <- Ash.read(total_count_query),
         ...
         {:ok, [%{avg: avg_height}]} <- Ash.read(avg_height_query),
         do: {:ok, %{
           total_count: total_count,
