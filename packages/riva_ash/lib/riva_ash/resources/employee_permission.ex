defmodule RivaAsh.Resources.EmployeePermission do
  ...
  relationships do
    ...
    def is_active?(employee_permission) do
      with %{archived_at: nil} <- employee_permission do
        true
      else
        _ -> false
      end
    ...
    def formatted_assignment(employee_permission) do
      with true <- is_active?(employee_permission),
           ...
           notes when is_binary(notes) <- employee_permission.notes do
        ...
        nil ->
          "#{employee_display_name(employee_permission)} was granted '#{permission_name(employee_permission)}' by #{granter_display_name(employee_permission)}"
      end
    end
    ...
    def can_grant_permission?(employee_id, permission_id, granted_by_id) do
      with {:ok, true} <- validate_employee_business_match(employee_id, granted_by_id),
           {:ok, true} <- validate_permission_exists(permission_id) do
        {:ok, true}
      else
